type Resource = {
  title: string;
  url: string;
  platform?: string;
  channel?: string;
  resource_type?: string;
  authors?: string[];
  language?: string;
  thumbnail?: string;
};

type ResourceGroup = { title: string; resources: Resource[] };

interface ResourceGridProps {
  groups: ResourceGroup[];
}

export default function ResourceGrid({ groups }: ResourceGridProps) {
  if (!groups.length) return null;

  const getFavicon = (url: string) => {
    try {
      const { hostname } = new URL(url);
      return `https://www.google.com/s2/favicons?domain=${hostname}&sz=128`;
    } catch {
      return `https://www.google.com/s2/favicons?domain=example.com&sz=128`;
    }
  };

  const getFormatFromUrl = (url: string) => {
    try {
      const p = new URL(url).pathname.toLowerCase();
      const m = p.match(/\.([a-z0-9]+)(?:$|\?)/);
      return m ? m[1] : 'html';
    } catch {
      return 'html';
    }
  };

  const getCategory = (r: Resource) => {
    const t = (r.resource_type || '').toLowerCase();
    const ext = getFormatFromUrl(r.url);
    if (t.includes('book') || t.includes('ebook') || ext === 'pdf' || ext === 'epub' || ext === 'mobi') return 'Book';
    if (t.includes('video') || t.includes('course') || t.includes('lecture') || ext === 'mp4' || ext === 'webm') return 'Video';
    return null;
  };

  return (
    <div className="space-y-6 mt-4">
      {groups.map((group) => (
        <section key={group.title}>
          <h2 className="text-xl font-semibold mb-3">{group.title}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {group.resources.map((resource, index) => (
              <a
                key={index}
                href={resource.url}
                target="_blank"
                rel="noopener noreferrer"
                className="group block p-4 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 transition-all duration-200 shadow-sm hover:shadow-md hover:-translate-y-0.5 backdrop-blur-sm"
              >
                {resource.thumbnail ? (
                  <div className="mb-3 overflow-hidden rounded-lg border border-white/10 relative">
                    <img
                      src={resource.thumbnail}
                      alt={resource.title}
                      className="w-full h-40 object-cover transition-transform duration-300 group-hover:scale-[1.02]"
                      loading="lazy"
                      referrerPolicy="no-referrer"
                    />
                    <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/20 to-transparent" />
                  </div>
                ) : (
                  <div className="mb-3">
                    <img
                      src={getFavicon(resource.url)}
                      alt=""
                      className="h-10 w-10 rounded-md ring-1 ring-white/15 shadow"
                      loading="lazy"
                    />
                  </div>
                )}
                <h3 className="text-base sm:text-lg font-semibold text-purple-300 break-words leading-snug">{resource.title}</h3>
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  {(() => {
                    const category = getCategory(resource);
                    return category ? (
                      <span className="inline-flex items-center rounded-md bg-purple-600/20 text-purple-300 border border-purple-500/20 px-2 py-0.5 text-xs">{category}</span>
                    ) : null;
                  })()}
                  {(() => {
                    const fmt = getFormatFromUrl(resource.url);
                    return fmt !== 'html' ? (
                      <span className="inline-flex items-center rounded-md bg-white/10 text-gray-200 border border-white/10 px-2 py-0.5 text-xs uppercase">{fmt}</span>
                    ) : null;
                  })()}
                </div>
                {resource.platform && (
                  <p className="text-xs text-gray-400 mt-1">Platform: {resource.platform}</p>
                )}
                {resource.resource_type && (
                  <p className="text-xs text-gray-400 mt-1">Type: {resource.resource_type}</p>
                )}
                {resource.authors && resource.authors.length > 0 && (
                  <p className="text-xs text-gray-400 mt-1">Authors: {resource.authors.join(', ')}</p>
                )}
              </a>
            ))}
          </div>
        </section>
      ))}
    </div>
  );
}
